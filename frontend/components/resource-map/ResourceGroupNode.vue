<template>
  <div
    class="rounded-lg border border-dashed pointer-events-none"
    :class="[
  ...
  >
    <div class="flex items-center gap-1.5 px-3 py-1.5 pointer-events-auto">
      <input
        v-if="data.editing"
        ref="nameInput"
        v-model="editValue"
...
function startEdit() {
  editValue.value = props.data.name;
  // Trigger editing via the parent's callback mechanism
  props.data.onStartEdit?.();
}
