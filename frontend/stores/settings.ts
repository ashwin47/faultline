...
export const useSettingsStore = defineStore('settings', () => {
  ...
  /**
   * Delete a specific integration instance and reindex remaining ones
   */
  async function deleteIntegrationInstance(integration: string, index: number): Promise<boolean> {
    try {
      await axios.delete(`${accountBase()}/settings/integration/${integration}/${index}`);
      await fetchSettings();
      await fetchIntegrationStatus();
      return true;
    } catch (err: any) {
      console.error(`Error deleting ${integration} instance ${index}:`, err);
