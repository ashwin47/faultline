    ...
    <div class="border-b border-gray-300 dark:border-gray-700 px-4 py-3 flex items-center justify-between shrink-0">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          class="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-red-300 dark:border-red-800 text-red-600 dark:text-red-400 hover:bg-red-600 hover:text-white dark:hover:bg-red-500 dark:hover:text-white transition-colors"
        >
  ...
  return groups.map((group) => {
    ...
    return {
      ...
      data: {
        ...
        onStartEdit: () => { editingGroupId.value = group.id; },
...
// Edge deletion via click (useVueFlow hook)
...
async function handleSync() {
  if (!currentMap.value) return;
  await store.syncMap(currentMap.value.id);
  if (currentMap.value && currentMap.value.nodes.length > 0) {
    handleReorganize();
  }
}
...
async function handleDeleteMap() {
  if (!currentMap.value) return;
  if (!confirm(`Delete "${currentMap.value.name}"? This cannot be undone.`)) return;
  await store.deleteMap(currentMap.value.id);
  router.replace('/resource-maps');
}
