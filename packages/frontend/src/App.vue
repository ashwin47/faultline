...
const { connect: socketConnect, disconnect: socketDisconnect, getSocket } = useSocket();
...
function listenForAccountEvents() {
  ...
  sock?.on('conversation:created', (data: { id: string; title: string | null; createdAt: string; updatedAt: string; messageCount: number }) => {
    agentStore.addConversationToList(data);
  });
  sock?.on('conversation:updated', (data: { id: string; title?: string; updatedAt: string }) => {
    agentStore.updateConversationInList(data);
  });
}
